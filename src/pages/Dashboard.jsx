...
export default function Dashboard() {
  ...
  function regenerateKey() {
    ...
    fetch(`${API_URL}/api/auth/regenerate`, {
      ...
      .then(r => {
        if (!r.ok) {
          return r.json().then(data => { throw new Error(data.error || 'Server error (' + r.status + ')') })
        }
        return r.json()
      })
      .then(data => {
      ...
      .catch(err => {
        alert('Regenerate failed: ' + (err.message || 'Network error. The Worker may need redeployment.'))
      })
  }
